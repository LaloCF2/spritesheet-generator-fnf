...
let atlasAnimObj = null;
let atlasJsonObj = null;
let atlasPngImg = null;
let atlasPngFileName = "";
let animSpritesDict = {}; 
...
function updateAtlasPreviewCanvas() {
    ...
    let canvas = document.getElementById('atlasPreviewCanvas');
    // Resolución fija razonable para preview
    ...
    // Dibujar el primer cuadro de la animación
...
async function ejecutarAtlasBaker() {
    ...
    showLoader("HORNEANDO ATLAS", "Generando fotogramas... 0%");
    
    // 1. Extraer definiciones de animacion principal
    let animations = [];
    let layers = Array.isArray(atlasAnimObj.AN.TL.L) ? atlasAnimObj.AN.TL.L : [atlasAnimObj.AN.TL.L];
    layers.forEach(l => {
        let frames = Array.isArray(l.FR) ? l.FR : [l.FR];
    ...
    let framesRendered = [];
    let scaleMultiplier = 1; // Ajustable si es necesario
    ...
    for(let a=0; a<animations.length; a++) {
        ...
        for(let f=0; f<frameCount; f++) {
            ...
            function updateBounds(x, y) { if(x<minX)minX=x; if(x>maxX)maxX=x; if(y<minY)minY=y; if(y>maxY)maxY=y; }
            
            // Dibujar a un canvas gigante primero (para calcular bounds)
            let tempCanv = document.createElement('canvas');
            tempCanv.width = 2000; tempCanv.height = 2000;
            let tCtx = tempCanv.getContext('2d');
            tCtx.translate(1000, 1000); // Origen en el centro
            ...
            for(let y=0; y<2000; y++){
                for(let x=0; x<2000; x++){
                    let alpha = d[(y*2000+x)*4+3];
                    if(alpha > 5) { updateBounds(x,y); }
                }
            }
    ...
    let xml = '<?xml version="1.0" encoding="utf-8"?>\n<TextureAtlas imagePath="'+finalPngName+'">\n';
    ...
    let pngBlob = null;
    if(optPng || optOpt) { // Si se pide PNG
        if(optOpt) {
    ...
    let box = document.getElementById('atlasPreviewBox');
    box.innerHTML = `
        <div style="font-size:2rem; font-weight:bold; color:var(--accent-gold);">¡HORNEADO!</div>
...
function renderSymbolTimeline(tl, frameIndex, ctx) {
    ...
    // Dibujar de abajo hacia arriba en Animate (Z-index invertido)
